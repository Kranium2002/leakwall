...
// Config struct fields match the leakwall.toml schema; not all are consumed by CLI yet.
...
impl Default for LeakwallConfig {
    fn default() -> Self {
        Self {
...
pub fn load_config(explicit_path: Option<&PathBuf>) -> LeakwallConfig {
    ...
    // Try ./leakwall.toml (project-level)
    let local = PathBuf::from("leakwall.toml");
    if let Some(config) = try_load_config(&local) {
        debug!(path = %local.display(), "loaded project config");
        return config;
    }

    // Try ~/.leakwall/config.toml (global)
    if let Some(home) = dirs::home_dir() {
        let global = home.join(".leakwall/config.toml");
    ...
    LeakwallConfig::default()
}

fn try_load_config(path: &std::path::Path) -> Option<LeakwallConfig> {
    let content = std::fs::read_to_string(path).ok()?;
    match toml::from_str::<LeakwallConfig>(&content) {
...
mod tests {
    ...
    fn test_default_config() {
        let config = LeakwallConfig::default();
...
cache_ttl_hours = 12
"#;
        let config: LeakwallConfig = toml::from_str(toml_str).unwrap();
