...
/// Returns the AgentAudit base URL, checking `AEGIS_AGENTAUDIT_URL` env var first.
fn agentaudit_base_url() -> String {
    std::env::var("AEGIS_AGENTAUDIT_URL").unwrap_or_else(|_| AGENTAUDIT_BASE_URL.to_string())
}

/// Build a reqwest client with standard Aegis settings.
...
/// Catalog cache path: `~/.aegis/package_catalog.json`.
fn catalog_path() -> Option<PathBuf> {
    Some(dirs::home_dir()?.join(".aegis/package_catalog.json"))
}
...
mod tests {
    ...
    /// Serialize async tests that mutate `AEGIS_AGENTAUDIT_URL` so parallel threads
    ...
    async fn test_check_existing_package() {
        ...
        std::env::set_var("AEGIS_AGENTAUDIT_URL", format!("{}/api", mock_server.uri()));

        let result = query_agentaudit("mcp-remote").await;

        // Clean up env var
        std::env::remove_var("AEGIS_AGENTAUDIT_URL");
