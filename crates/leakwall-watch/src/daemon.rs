...
/// Uses `/proc/{pid}` which is Linux-specific (sufficient for LeakWall's
...
pub async fn start_daemon(
    ...
    info!(pid = std::process::id(), "leakwall daemon started");
...
mod tests {
    ...
    fn test_pid_file_write_read() {
        ...
        let pid_path = tmp.path().join("leakwall.pid");
    ...
    fn test_read_pid_file_missing() {
        let path = Path::new("/tmp/leakwall_nonexistent_pid_xyz");
