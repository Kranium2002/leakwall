...
/// Create a combined CA bundle containing system CAs + the Aegis CA.
///
/// The combined bundle is written to `aegis_dir/ca-bundle.pem`.  If the system
/// bundle cannot be found, falls back to just the Aegis CA with a warning.
fn create_combined_ca_bundle(
    ca_cert_path: &Path,
    aegis_dir: &Path,
) -> Result<std::path::PathBuf, ProxyError> {
    let aegis_ca = std::fs::read_to_string(ca_cert_path)
        .map_err(|e| ProxyError::ProcessSpawn(format!("read aegis CA: {e}")))?;
    ...
    let combined = match system_bundle {
        ...
        None => {
            warn!("system CA bundle not found, using Aegis CA only");
            aegis_ca
        }
    };

    let bundle_path = aegis_dir.join("ca-bundle.pem");
...
pub fn spawn_agent(
    ...
    let proxy_url = format!("http://aegis:{proxy_token}@127.0.0.1:{proxy_port}");
    ...
    cmd.env("AEGIS_PROXY_TOKEN", proxy_token);

    // Create combined CA bundle (system CAs + Aegis CA)
    let ca_path = ca_cert_path.to_string_lossy().to_string();
    let bundle_path = create_combined_ca_bundle(ca_cert_path, aegis_dir)
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_else(|e| {
            warn!(error = %e, "failed to create combined CA bundle, using aegis CA only");
            ca_path.clone()
        });
