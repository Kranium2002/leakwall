use aegis_secrets::scanner::{SecretMatch, SecretScanner};
use bytes::Bytes;
use std::sync::Arc;
...
/// with `[AEGIS:<pattern_name>:REDACTED]` markers.
///
...
mod tests {
    use super::*;
    use aegis_secrets::scanner::MatchSource;
    use aegis_secrets::Severity;
    ...
    fn test_single_redaction() {
        ...
        assert!(result.contains("[AEGIS:aws_access_key:REDACTED]"));
    ...
    fn test_multiple_redactions() {
        ...
        let redacted = redact_body(body, &matches);
        let result = String::from_utf8(redacted).unwrap();
        assert!(result.contains("[AEGIS:key1:REDACTED]"));
        assert!(result.contains("[AEGIS:key2:REDACTED]"));
