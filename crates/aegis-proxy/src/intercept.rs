...
pub async fn handle_intercepted_request(
    ...
    let scan_result = if skip_scan {
        aegis_secrets::scanner::ScanResult {
            matches: vec![],
            scan_duration: std::time::Duration::ZERO,
            body_size,
        }
        ...
        tokio::task::spawn_blocking(move || scanner.scan(&scan_bytes))
            .await
            .unwrap_or_else(|_| aegis_secrets::scanner::ScanResult {
                matches: vec![],
                scan_duration: std::time::Duration::ZERO,
                body_size,
            })
        ...
        match current_mode {
            ...
            ScanMode::Block => {
                ...
                return Ok(Response::builder()
                    .status(403)
                    .body(full_body(Bytes::from(
                        "[AEGIS] Request blocked — secret exfiltration detected",
                    )))
                    .unwrap_or_else(|_| Response::new(empty_body())));
            }
        }
    ...
    match forward_result {
        ...
        Err(e) => {
            ...
            Ok(Response::builder()
                .status(502)
                .body(full_body(Bytes::from(format!("[AEGIS] Proxy error: {e}"))))
                .unwrap_or_else(|_| Response::new(empty_body())))
        }
    }
}
                    ...
                    Ok(chunk) => {
                        ...
                        if should_block {
                            ...
                            let err_frame = Frame::data(Bytes::from(
                                "[AEGIS] Response blocked - secret detected in stream",
                            ));
                ...
                match &current_mode {
                    ...
                    ScanMode::Block => {
                        ...
                        return Ok(Response::builder()
                            .status(502)
                            .body(full_body(Bytes::from(
                                "[AEGIS] Response blocked - secret detected",
                            )))
                            .unwrap_or_else(|_| Response::new(empty_body())));
                    }
                }
