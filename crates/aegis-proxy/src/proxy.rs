...
/// Verify proxy authentication via Basic auth (username "aegis", password = proxy_token).
fn check_proxy_auth(req: &Request<hyper::body::Incoming>, expected_token: &str) -> bool {
    ...
    // Expect "Basic <base64(aegis:token)>"
...
async fn handle_request(
    ...
    if !check_proxy_auth(&req, &state.proxy_token) {
        ...
        return Ok(Response::builder()
            .status(407)
            .header("Proxy-Authenticate", "Basic realm=\"aegis-proxy\"")
            .body(full_body(Bytes::from(
                "[AEGIS] Proxy authentication required",
            )))
