...
use aegis_tui::events::create_event_channel;
use aegis_tui::report::{build_session_report, print_session_report, save_session_report};
...
/// Run the `aegis run` command — proxy + TUI + scanner.
pub async fn run_proxy(
    ...
    let (ca_cert_pem, ca_key_pem) =
        load_or_generate_ca(aegis_dir).context("CA certificate setup failed")?;
    let ca_cert_path = aegis_dir.join("ca.pem");

    // 2. Build the secret scanner
    let scanner = build_scanner(aegis_dir)?;
    ...
    let log_dir = aegis_dir.join("logs");
...
fn build_scanner(aegis_dir: &Path) -> Result<SecretScanner> {
    // Discover secrets inline instead of loading from DB
    let home = dirs::home_dir().unwrap_or_default();
    let cwd = std::env::current_dir().unwrap_or_default();

    let secrets = aegis_secrets::discovery::discover_secrets(&home, &cwd)
    ...
    // Load regex patterns
    let patterns_path = aegis_dir.join("patterns.toml");
