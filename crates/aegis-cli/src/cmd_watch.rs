use aegis_watch::{DaemonConfig, DaemonRequest, WatchConfig};
use anyhow::{Context, Result};
use colored::Colorize;
use std::path::Path;

/// Run the `aegis watch` command — continuous monitoring daemon.
pub async fn run_watch(
    ...
    let config = DaemonConfig {
        pid_file: aegis_dir.join("aegis.pid"),
        log_file: aegis_dir.join("daemon.log"),
        socket_path: aegis_dir.join("aegis.sock"),
    };
    ...
    let watch_config = build_watch_config(aegis_dir)?;
    ...
    if daemon || foreground {
        if !quiet {
            println!("{}", "Starting aegis daemon...".bold());
            println!(
        ...
        aegis_watch::daemon::start_daemon(&config, watch_config)
            .await
            .context("daemon failed")?;
    } else {
        println!(
            "{}",
            "Usage: aegis watch --daemon | --status | --stop".dimmed()
        );
...
async fn query_status(config: &DaemonConfig) -> Result<()> {
    let response =
        aegis_watch::daemon::send_daemon_command(&config.socket_path, DaemonRequest::Status)
            .await
            .context("aegis daemon not running. Start with: aegis watch --daemon")?;

    if response.success {
        println!("{}", "aegis daemon running".green().bold());
...
async fn stop_daemon(config: &DaemonConfig) -> Result<()> {
    let response =
        aegis_watch::daemon::send_daemon_command(&config.socket_path, DaemonRequest::Stop)
            .await
            .context("aegis daemon not running. Nothing to stop.")?;

    if response.success {
        println!("{}", "aegis daemon stopped.".green().bold());
    } else {
        println!(
            "{}: {}",
            "Error".red(),
...
fn build_watch_config(aegis_dir: &Path) -> Result<WatchConfig> {
    let home = dirs::home_dir().unwrap_or_default();
    let cwd = std::env::current_dir().unwrap_or_default();

    // Discover MCP config paths
    let mcp_configs = aegis_mcp::discover::discover_mcp_configs()
        .unwrap_or_default()
        .into_iter()
        .map(|loc| loc.path)
        .collect::<Vec<_>>();
    ...
    Ok(WatchConfig {
        ...
        tool_hash_file: aegis_dir.join("tool_hashes.json"),
    })
}
