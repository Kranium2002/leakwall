use aegis_report::html::render_html_report;
use aegis_report::json::{Finding, ReportSummary, ScanReport};
use aegis_report::sarif::generate_sarif;
use anyhow::{Context, Result};
use colored::Colorize;
use std::path::Path;

/// Run the `aegis report` command — generate reports from scan data.
pub async fn run_report(
    ...
    let report = load_latest_report(aegis_dir)?;
    ...
    match format {
        ...
        Some("json") => {
            let json = aegis_report::json::generate_json_report(&report)
        ...
        _ => {
            println!(
                "{}",
                "Usage: aegis report --html [--open] | --format sarif|json".dimmed()
            );
        }
    }
...
fn load_latest_report(aegis_dir: &Path) -> Result<ScanReport> {
    let report_dir = aegis_dir.join("reports");
