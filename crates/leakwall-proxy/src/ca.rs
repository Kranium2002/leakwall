...
pub fn generate_ca() -> Result<(String, Zeroizing<String>), ProxyError> {
    ...
    params
        .distinguished_name
        .push(DnType::CommonName, "LeakWall Local CA");
    params
        .distinguished_name
        .push(DnType::OrganizationName, "LeakWall Security");
...
/// Load or generate CA certificate, saving to ~/.leakwall/.
pub fn load_or_generate_ca(lw_dir: &Path) -> Result<(String, Zeroizing<String>), ProxyError> {
    let cert_path = lw_dir.join("ca.pem");
    let key_path = lw_dir.join("ca-key.pem");
    ...
    // Generate new CA
    std::fs::create_dir_all(lw_dir).map_err(|e| ProxyError::CaError(format!("create dir: {e}")))?;
